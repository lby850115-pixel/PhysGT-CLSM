...
for ax, (col, label) in zip(axes.flat, METRICS):
    ...
    cbar.set_label('-log₁₀(p-value)', fontsize=7, rotation=270, labelpad=15)
    cbar.ax.tick_params(labelsize=6)

    # Add reference lines on colorbar for significance thresholds
    # p=0.05 → -log10(0.05)=1.3, p=0.01 → -log10(0.01)=2, p=0.001 → -log10(0.001)=3
    cbar.ax.axhline(y=1.3, color='black', linewidth=0.5, linestyle='--', alpha=0.5)
    cbar.ax.axhline(y=2.0, color='black', linewidth=0.5, linestyle='--', alpha=0.5)
    cbar.ax.axhline(y=3.0, color='black', linewidth=0.5, linestyle='--', alpha=0.5)
